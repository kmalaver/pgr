...
func (i *interpolator) encodePlaceholder(value interface{}, topLevel bool) error {
	if builder, ok := value.(Builder); ok {
		pbuf := NewBuffer()
		err := builder.Build(pbuf)
		if err != nil {
			return err
		}
		paren := false
		switch value.(type) {
		case *SelectBuilder, *union:
			paren = !topLevel
		}
		if paren {
			i.WriteString("(")
	...
	switch v.Kind() {
	case reflect.String:
		i.WriteString(EncodeString(v.String()))
		return nil
	case reflect.Bool:
		i.WriteString(EncodeBool(v.Bool()))
		return nil
	...
	case reflect.Struct:
		if v.Type() == typeTime {
			i.WriteString(EncodeTime(v.Interface().(time.Time)))
			return nil
		}
	case reflect.Slice:
		if v.Type().Elem().Kind() == reflect.Uint8 {
			// []byte
			i.WriteString(EncodeBytes(v.Bytes()))
			return nil
		}
		if v.Len() == 0 {
